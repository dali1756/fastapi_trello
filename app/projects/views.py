...
async def update(project_name: str, name: Annotated[str, Form()], request: Request, description: Annotated[Optional[str], Form()] = None, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    project_to_update = db.query(Project).join(UserProject, UserProject.project_id == Project.id).filter(Project.name == project_name, UserProject.user_id == current_user.id).first()
    if not project_to_update:
        raise HTTPException(status_code=404, detail="查無專案。")
    existing_project = db.query(Project).filter(Project.name == update_data.name, Project.id != project_to_update.id).first()
    if existing_project:
        if request.headers.get("HX-Request") == "true":
            ...
            raise HTTPException(status_code=400, detail="該專案名稱已被使用。")
    project_to_update.name = update_data.name
    if description is not None:
        project_to_update.description = update_data.description
    db.commit()
    is_htmx = request.headers.get("HX-Request") == "true"
    if is_htmx:
        content = templates.get_template("projects/partials/projects_show.html").render({"request": request, "projects": project_to_update, "current_user": current_user})
        return HTMLResponse(content=content)
    else:
        return RedirectResponse(url="/projects", status_code=status.HTTP_302_FOUND)
...
async def show(request: Request, project_name: str, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    project = db.query(Project).join(UserProject, UserProject.project_id == Project.id).filter(Project.name == project_name, UserProject.user_id == current_user.id).first()
    if not project:
        raise HTTPException(status_code=404, detail="查無專案。")
...
async def delete(project_name: str, request: Request, current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    project_to_delete = db.query(Project).join(UserProject, UserProject.project_id == Project.id).filter(Project.name == project_name, UserProject.user_id == current_user.id).first()
    if project_to_delete:
        db.query(UserProject).filter(UserProject.project_id == project_to_delete.id).delete()
